 ...
 *
 * Not all calls are implemented but should be easy to add
 * All API call methods start with an "api"
 * Flow: construct with creds -> api call with params -> curlCall -> results as array
 *
 * History:
 * By Nik Stankovic, 2016-10-12, Version 1: only price API
 *                   2016-10-13, Version 2: account info, new order, etc CURL incl POST, DELETE
 *
 */
...
class OandaAPI {
     ...
     *
     * @returns an array of instrument prices, sample JSON response that gets converted:
     *
        {
          "prices": [
            {
     ...
     * price: Required If order type is ‘limit’, ‘stop’, or ‘marketIfTouched’. The price where the 
     *        order is set to trigger at.
     *
     * OPTIONAL PARAMETERS:
